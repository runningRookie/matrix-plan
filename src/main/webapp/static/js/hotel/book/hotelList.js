...
var book1 = {
	model : {
		...
		hotelbuttontype:"0" //酒店类型按钮，默认值为会员酒店；0：会员酒店，1：协议酒店
	...
	hotelListDate : function(type,pageNum,sectionId){//pageNum:分页
		...
		if(this.model.hotelbuttontype==0){
		$.ajax({
			type:"POST",
...
//					var xinjiName = {"27":"二星级","26":"三星级","24":"四星级","23":"五星级","28":"豪华型","30":"高档型","31":"舒适型","32":"经济型","15":"酒店式公寓"};
		...
		}else if(this.model.hotelbuttontype==1){
			
			//预订人与乘客信息；
			if(_this.model.bookPersonAndPassengers){
				requestData.bookPersonAndPassengers = _this.model.bookPersonAndPassengers;
			}
			$.ajax({
				type:"POST",
				url : __ctx+"/hotel/getAgreeHotelList",
				data :JSON.stringify(requestData),
				contentType: "application/json; charset=utf-8",
				dataType:"json",
				async:true,
...
//		$("#hotel-getcity").css({"top":(wz.top+_height)+"px","left":wz.left+"px"});
		...
		$("#hotel-getcity").on("click","li",function(){
			...
			if(type==0){//选择的是城市	
				...
				if($this.attr("data-keyname")){
					$(".select-city>p").text($this.attr("data-keyname"));
...
//		$("#hotel-getkey").css({"top":(wz.top+_height)+"px","left":wz.left+"px"});
		...
		$("#hotel-getkey").css({"top":top+"px","left":left+"px"});
			...
			if(_this.model.hotelbuttontype==0){
				$.ajax({
					type:"POST",
			...
			}else if(_this.model.hotelbuttontype==1){
				...
				$.ajax({
					type:"POST",
					url:__ctx+"/common/getAgreeKeywords",
					data:JSON.stringify(requestData),
					contentType: "application/json; charset=utf-8",
					dataType:"json",
					async:true,
...
//		});		
	...
	book1ClickFunc : function(){
		...
		$(".hotel-list-left").on("click",".list-btn",function(){
	            ...
	            if(_this.model.hotelbuttontype==0){
				   document.form1.action= __ctx+"/hotel/detail";
	            }else if(_this.model.hotelbuttontype==1){
	               document.form1.action= __ctx+"/hotel/agreeHotelDetail";
	            }
				$("#formid").submit();		
		});
		//会员酒店按钮点击
		$("#tmchotelbtn").click(function(){
			book1.model.hotelbuttontype=0;
		});
		
		//协议酒店按钮点击
		$("#agreehotelbtn").click(function(){
			book1.model.hotelbuttontype=1;
		});
		
		$(".hotel-details-menu>a").click(function(){
			var $this = $(this);
			if($this.hasClass("hotel-details-menu-act")){
				return;
			}
			$(".hotel-details-menu>a").removeClass("hotel-details-menu-act");
			$this.addClass("hotel-details-menu-act");
			//清空筛选条件重新加载
			_this.model.keyword = "";					
			$("#input-key").val("");
			_this.reductionSec();
