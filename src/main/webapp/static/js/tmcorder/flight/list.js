    ...
    var resultVM = new Vue({
        ...
        ready: function () {
            ...
            $.getJSON(__ctx + '/servicefee/da', {
                serviceType: 1,
                companyId: window.bookPersonAndPassengers.bookPerson.bookCompanyId,
                policyType: '45'
            }, function (data) {
                thisVM.serviceFee = (data.obj && data.obj.amount) || 0;
            });
            $.getJSON(__ctx + '/servicefee/da', {
                serviceType: 1,
                companyId: window.bookPersonAndPassengers.bookPerson.bookCompanyId
            }, function (data) {
                thisVM.unofficialServiceFee = (data.obj && data.obj.amount) || 0;
            });
        ...
        methods: {
            ...
            order: function (flight, cabin, isAllowBooking, pIndex, index) {
                if (!!cabin && !isAllowBooking) {
                    toastr.error("所选择的乘客被禁止预定", "", toastrConfig);
                    return;
                }
                ...
                var employeeIds = _.filter(_.map(window.bookPersonAndPassengers.passengers, function (item) {
                    return item.passengerEmployeeId;
                })).toString();

                $.ajax({
                    type: "GET",
                    url: __ctx + "/employees/airTravelProtocols",
                    data: {employeeIds: employeeIds},
                    async: false,
                    success: function (result) {
                        travelPolicyModalVM.airTravelProtocols = result.obj;

                        if (result.result && !!result.obj) {
                            passengerHourNums = genLeastHourNum(result.obj);
                        }
                        ...
                        if (tc.arr.isNotEmpty(violationContents) || cabin.flightData.isViolatePolicy) {
                            travelPolicyModalVM.init(flight, cabin, briefs, violationContents, passengerHourNums, cctInsuranceSaveDTOS);
                            $('#travelPolicyModalVM').show();
                            return;
                        }
        ...
        methods: {
            init: function (selectFlight, selectCabin, selectBriefs, violationContents, passengerHourNums, cctInsuranceSaveDTOS) {
                ...
                var filter = function (cabin) {
                    ...
                    if (!cabin.flightData.isAllowBooking) {
                        return true;
                    }
                    return false;
                };
                ...
                if (!leastCabin.cabin) {
                    goFlightConfirm(selectFlight, selectCabin, selectBriefs, passengerHourNums, null, cctInsuranceSaveDTOS);
                }
...
//                    var url = baseUrl + (suggest.cabin.flightData.isAgreeFare === 'Y' ? 'protocolrefund' : 'flightrefund');
    ...
    function search(params) {
        ...
        var passengers = {
            bookPerson: window.bookPersonAndPassengers.bookPerson,
            passengers: window.bookPersonAndPassengers.passengers
        };
        params = _.assign(params, passengers);
        $.ajax({
            type: "POST",
            contentType: "application/json",
            url: baseUrl,
    ...
    var searchVM = new Vue({
        ...
        ready: function () {
            ...
            if (!!window.listPageRequest.depCode) {
                thisVM.params.depCodeAndName = window.listPageRequest.depCode + ',' + window.listPageRequest.depName;
                thisVM.params.depCode = window.listPageRequest.depCode;
                thisVM.params.depName = window.listPageRequest.depName;
                thisVM.params.arrCodeAndName = window.listPageRequest.arrCode + ',' + window.listPageRequest.arrName;
                thisVM.params.arrCode = window.listPageRequest.arrCode;
                thisVM.params.arrName = window.listPageRequest.arrName;
                thisVM.params.depDate = window.listPageRequest.depDate;
                thisVM.params.backDate = window.listPageRequest.backDate;
                thisVM.params.travelType = window.listPageRequest.travelType;
                setTimeout(thisVM.search, 1000);
            }
        },
        methods: {
            search: function () {
            ...
            goBack: function () {
                window.location = __ctx + "/itinerary/bookform?reSelect=Y";
            }
        }
    });
        ...
        _.forEach(allFlights, function (flight) {
            ...
            if (!hourNum || (before <= currentFlightDepTime && currentFlightDepTime <= after)) {
                _.forEach(flight.cabins, function (cabin) {
                    ...
                    if (!leastCabin.cabin || leastPrice > cabin.flightData.fare) {
    ...
    function initViolationPassenger(passengers, cabin, violationContents) {
        ...
        _.forEach(passengers, function (passenger) {
            ...
            _.forEach(policys, function (item) {
                if (item.passengerId == passenger.passengerEmployeeId && tc.arr.isNotEmpty(item.policyContent)) {
                    arr.push({
                        passengerId: item.passengerId,
                        passengerName: item.passengerName
                    });
