...
$(document).ready(function () {
    Vue.component('vue-select', {
        template: '<select v-bind:class="class" v-model="model" v-bind:multiple="multiple" v-bind:size="size" v-bind:name="name" v-bind:id="id" v-bind:disabled="disabled" v-on:change="changeCompany">' +
        '<option value>--请选择--</option>' +
        '<option v-for="item in formattedOptions" value="{{item.value}}">{{item.text}}</option>' +
        '</select>',
        props: {
            id: {
                type: String,
        ...
        watch: {
            "options": function (val, oldVal) {
                var thisVM = this;
                thisVM.formattedOptions = _.map(val, function (item) {
                    var value = _.map(thisVM.value.split(','), function (key) {
                        return _.get(item, key);
                    }).toString();
                    var text = _.map(thisVM.text.split(','), function (key) {
                        return _.get(item, key);
                    }).toString();
        ...
        ready: function () {
            ...
            this.control.on("change", function (e) {
                if (!me.isChanging || me.isModelChanging) {
                    ...
                    me.$dispatch('changeCompany', {"company": this})
                }
            });
        }
    });
