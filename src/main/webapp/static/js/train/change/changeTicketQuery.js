...
$(function () {
    ...
    var limitDate = {"start":"","end":""};
    function getPlanTime(){
        $.ajax({
            url: __ctx + "/train/change/planTime",
            type: 'POST',
            datatype: "json",
            contentType: "application/json;charset=utf-8",
            async: false,
            data: JSON.stringify({
                "orderItemId": sessionStorage.itemId
            }),
            success: function (dataRes) {
                limitDate.start = dataRes.obj.beginTime;
                limitDate.end = dataRes.obj.endTime;
            },
            error: function(){
                toastr.error("日期查询失败","", toastrConfig);
            }
        });
    }
    getPlanTime();
    ...
    var val1 = sessionStorage.fromStation;
    $("#fromStation").val(val1).trigger("change");
    $($("#fromStation").prev().children("a").children("span")[0]).text(val1);
    $("#fromStation").prop("disabled", true);

    var val2 = sessionStorage.toStation;
    $("#toStation").val(val2).trigger("change");
    $($("#toStation").prev().children("a").children("span")[0]).text(val2);
    ...
    $("#datetimepicker").datetimepicker({
        ...
        "changeDate", function () {
            $("#datetimepicker").datetimepicker({"setStartDate": limitDate.start, "setEndDate": limitDate.end});
        }
    );
    ...
    trainQueryVue = new Vue({
        ...
        data: {
            "searchParam": {
                "fromStation": sessionStorage.fromCity,
                "toStation": sessionStorage.toCity,
                "trainDate": "",
                "froms": [],
                "tos": [],
                "employeeIds": ""
            ...
            "stopOverResult": [],
            "planBeginDate": sessionStorage.planBeginDate,
        ...
        methods: {
            ...
            search: function () {
                ...
                book1.getTrainListAjax(trainQueryVue.searchParam.fromStation, trainQueryVue.searchParam.toStation, trainQueryVue.searchParam.trainDate, employeeIds, 1);
            },
            stopOverSearch: function (trainNo, fromStation, toStation, date, index) {
                var param = this;
                // param.stopOverResult = book1.stopOverSearch(trainNo, param.searchParam.fromStation, param.searchParam.toStation);
                param.stopOverResult = book1.stopOverSearch(trainNo, fromStation, toStation, date);
                param.indexNumber= index;
            },
            pdIsReserved: function (trainObj, ticketObj) {
                var param = this;
                var employeeNum = employeeIds.split(",").length;
                if (ticketObj.originTicket) {
                    toastr.warning("原坐席，不可预订", "", {
                        "positionClass": "toast-top-center"
                    });
                } else if (ticketObj.seats < employeeNum) {
                    toastr.warning("余票不足，不可预订", "", {
                        "positionClass": "toast-top-center"
                    });
                } else if (ticketObj.isViolate == '1' && ticketObj.canBook == 0) {
                    toastr.warning("违反差旅政策，不可预订!", "", {
                        "positionClass": "toast-top-center"
                    });
                } else {
                    var trainDate = this.searchParam.trainDate;

                    var trainEndDate = moment(trainDate + " " +  trainObj.fromTime).add(trainObj.runTimeSpan, 'minutes').format('YYYY-MM-DD HH:mm');
                    var trainBeginDate = moment(trainDate + " " +  trainObj.fromTime).format('YYYY-MM-DD HH:mm');
                    var reservedData = {
                        "queryKey": sessionStorage.queryKey,
                        "passengers": passengers,
                        "trainNo": trainObj.trainNo,
                        "trainClass": trainObj.trainClass,
                        "fromStationCode": trainObj.fromStationCode,
                        "fromStation": trainObj.fromStation,
                        "fromCityId": trainObj.fromCityId,
                        "fromCityName": trainObj.fromCityName,
                        "fromProvinceId": trainObj.fromProvinceId,
                        "fromProvinceName": trainObj.fromProvinceName,
                        "toStationCode": trainObj.toStationCode,
                        "toStation": trainObj.toStation,
                        "toCityId": trainObj.toCityId,
                        "toCityName": trainObj.toCityName,
                        "toProvinceId": trainObj.toProvinceId,
                        "toProvinceName": trainObj.toProvinceName,
                        "trainBeginDate": trainBeginDate,
                        "trainEndDate": trainEndDate,
                        "fromTime": trainObj.fromTime,
                        "runTimeSpan": trainObj.runTimeSpan,
                        "toTime": trainObj.toTime,
                        "miles": trainObj.miles,
                        "seatClass": ticketObj.seatClass,
                        "seatName": ticketObj.seatName,
                        "price": ticketObj.price,
                        "isViolate": ticketObj.isViolate,
                        "violateContent": ticketObj.violateContent,
                        "employeeViolateProtocolDTOList": ticketObj.employeeViolateProtocolDTOList,
                        "trainReservedPassengerDTO": passenger
                    };
                    sessionStorage.queryDate = JSON.stringify(reservedData);
                    window.history.go(-1);
                }
            }
        }
    ...
    var book1 = {
        ...
        init: function () {
            ...
            if (sessionStorage.result != null && sessionStorage.searchParam != null) {
                trainQueryVue.searchParam = JSON.parse(sessionStorage.searchParam);
                trainQueryVue.result = JSON.parse(sessionStorage.result);
                $("#trainQuery").show();
            } else {
                $("#trainQuery").hide();
            }
        ...
        dateShow: function () {
            var _this = this;
            //左右选择时间功能
            var startDate = new Date(new Date(limitDate.start).getTime() - 5 * 24 * 3600 * 1000);
            var endDate = new Date(new Date(limitDate.end).getTime() + 5 * 24 * 3600 * 1000);
        ...
        //查询车次
        getTrainListAjax: function (fromStation, toStation, trainDate, employeeIdStr, type) {
            $("#trainQuery").show();
            $("#loading").css({
                "display": "block"
            });
            ...
            $.ajax({
                ...
                success: function (dataRes) {
                    ...
                    //加工违反的差旅政策
                    var violateContent = function (content) {
                        return content.split("#");
                    };

                    //原车票信息
                    var originParam = JSON.parse(sessionStorage.param);
                    //日期转换
                    originParam.planBeginDate = originParam.planBeginDate.substring(0,10);

                    if (data != null && data.trains != null) {
                        //与原车票同一天
                        if (originParam.planBeginDate != undefined && originParam.trainNo != undefined && originParam.seatClass != undefined) {
                            if ($("#datetimepicker").val() != undefined && originParam.planBeginDate == $("#datetimepicker").val() && trainQueryVue.result.trains.length > 0) {
                                var trains = data.trains;
                                for (var a = 0; a < trains.length; a++) {
                                    //与原车票同一车次
                                    if (originParam.trainNo == trains[a].trainNo) {
                                        var tickets = trains[a].tickets;
                                        trains[a].originTrain = true;
                                        for ( var key in tickets) {
                                            // if (originParam.seatClass == tickets[key].seatName) {
                                            if (originParam.seatClass.indexOf(tickets[key].seatName) >= 0) {
                                                tickets[key].originTicket = true;
                                            }
                                        }
                                    }
                                }
                        ...
                        for (var i = 0; i < data.trains.length; i++) {
                            ...
                            $.each(tickets, function (index, obj) {
                                ...
                                if (obj.isViolate == 1) {
                                    obj.violateContentArr = violateContent(obj.violateContent);
                                }
                            });
                        }
                    }
    ...
    $("#search").on("click", function () {
        ...
        book1.getTrainListAjax(fromStation, toStation, $("#datetimepicker").val(), employeeIds, 0);
    });

    $("#backPage").on("click", function () {
        window.location.href = __ctx + "/trainorder/bookformTrain?flag=1";
    });
